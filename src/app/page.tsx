...
// src/app/page.tsx (New Authentication Screen matching design images 1 & 2)
"use client";
...
const AppleIcon = () => (
  <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-5 h-5 mr-3">
...
const GoogleIcon = () => (
  <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-3">
...
export default function WelcomePage() {
  return (
    <div className="relative flex flex-col items-center justify-end min-h-screen bg-background text-foreground p-4 pb-0 overflow-hidden">
      {/* Background Image Grid - matches screen 1 */}
      <div className="absolute inset-0 z-0">
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 gap-1 opacity-30 transform scale-110 blur-sm">
          {[...Array(49)].map((_, i) => (
            <div key={i} className="aspect-square bg-card rounded-sm overflow-hidden">
              <Image
                src={`https://placehold.co/200x200.png?text=Art${i+1}`}
                alt={`Placeholder art ${i+1}`}
                width={200}
                height={200}
      ...
      {/* Auth Panel - matches screen 2 */}
      <div className="relative z-10 bg-card rounded-t-2xl p-6 shadow-2xl w-full max-w-md mx-auto mb-0 mt-auto sm:mb-8 sm:rounded-xl sm:max-w-sm">
        <div className="text-center mb-6">
            {/* Optional: Logo or App Name can go here */}
            {/* <h1 className="text-2xl font-bold text-primary">Artifex</h1> */}
        </div>
        <div className="flex flex-col space-y-3">
          <Link href="/signup" passHref legacyBehavior>
            <Button asChild className="w-full h-12 text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl shadow-primary-glow">
              <a>Sign Up</a>
            </Button>
          </Link>
          
          <Link href="/login" passHref legacyBehavior>
            <Button asChild variant="outline" className="w-full h-12 text-sm font-semibold border-primary text-primary hover:bg-primary/10 hover:border-primary rounded-xl">
              <a>Login</a>
            </Button>
          </Link>
          ...
          <Button variant="outline" className="w-full h-12 text-sm text-foreground hover:bg-muted rounded-xl justify-center items-center border-muted hover:border-border">
            <AppleIcon />
            Continue with Apple
          </Button>
